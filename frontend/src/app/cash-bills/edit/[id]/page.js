...
export default function EditCashBillPage() {
  ...
  useEffect(() => {
    if (id) {
      loadBillAndAuxiliaryData();
    }
  }, [id]);

  const loadBillAndAuxiliaryData = async () => {
    try {
      const [billRes, custRes, prodRes] = await Promise.all([
        cashBillAPI.getById(id),
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await cashBillAPI.update(id, formData);
      ...
      <div className="flex-1 pt-14 md:pt-16 md:ml-64 overflow-auto">
        <div className="p-8">
          <div className="mb-8 flex justify-between items-center">
            <div>
              <div className="inline-block px-3 py-1 bg-indigo-100 border border-indigo-300 text-indigo-800 rounded-full text-xs font-bold uppercase tracking-wider mb-2">
                EDIT CASH BILL #{formData.billNumber}
              </div>
              <h1 className="text-4xl font-bold text-gray-900">Edit Cash / Service Bill</h1>
              <p className="text-gray-600">Modify particulars, rates, and client details</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 mb-6 border border-gray-100">
              ...
              {formData.items.length > 0 && (
                <div className="mt-6 flex flex-col md:flex-row justify-between items-center bg-gradient-to-r from-emerald-50 to-teal-50 p-6 rounded-2xl border border-emerald-200 gap-4">
                  <div>
                    <span className="text-xs uppercase font-bold tracking-wider text-emerald-800 block mb-1">Updated Receipt Total</span>
                    <p className="text-sm font-medium text-gray-600">
                      All line items calculated cleanly
                    </p>
                  </div>
            ...
            <div className="flex justify-end gap-4">
              ...
              >
                {saving ? 'Updating Cash Bill...' : '💾 Update Cash Bill'}
              </Button>
            </div>
          </form>
        </div>
