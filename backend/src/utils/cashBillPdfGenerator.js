...
export async function generateCashBillPDF(cashBill, company = null) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // --- TOP LEFT LOGO / BADGE ("SYSTEM DOCTOR") ---
      const logoX = startX + 8;
      const logoY = topY + 8;

      doc.save();
      // Draw SB Monogram
      doc.font('Helvetica-Bold').fontSize(22).fillColor('#1E3A8A');
      doc.text('SB', logoX, logoY);

      // Draw 'C' wrapping arc in correct direction (opening to the right)
      doc.lineWidth(2).strokeColor('#1E3A8A');
      doc.path(`M ${logoX + 42} ${logoY + 5} C ${logoX + 26} ${logoY + 3} ${logoX + 26} ${logoY + 23} ${logoX + 42} ${logoY + 21}`).stroke();

      // Mouse wire cord extending from C to mouse
      doc.lineWidth(1.2).strokeColor('#1E3A8A');
      doc.path(`M ${logoX + 42} ${logoY + 13} Q ${logoX + 52} ${logoY + 13} ${logoX + 52} ${logoY + 22}`).stroke();

      // Computer Mouse graphic
      doc.lineWidth(1).strokeColor('#1E3A8A');
      doc.ellipse(logoX + 52, logoY + 23, 4.5, 3.5).stroke();
      doc.moveTo(logoX + 52, logoY + 20).lineTo(logoX + 52, logoY + 22.5).stroke(); // Mouse button line

      // "SYSTEM DOCTOR" boxed badge underneath
      const badgeW = 58;
      const badgeH = 10;
      doc.lineWidth(0.8).strokeColor('#1E3A8A');
      doc.rect(logoX - 2, logoY + 25, badgeW, badgeH).stroke();
      doc.font('Helvetica-Bold').fontSize(5.5).fillColor('#1E3A8A');
      doc.text('SYSTEM DOCTOR', logoX - 2, logoY + 27, { width: badgeW, align: 'center' });
      doc.restore();
